# Comprehensive installer for cognis-digital/s3sniff (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# s3sniff is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "s3sniff"
$Url  = "git+https://github.com/cognis-digital/s3sniff.git"
$Git  = "https://github.com/cognis-digital/s3sniff.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: s3sniff"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: s3sniff"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: s3sniff"; exit 0 }
}
