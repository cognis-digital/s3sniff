"""Native cognis-connect emit for s3sniff — forward findings to any platform.

Maps s3sniff's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    s3sniff ... --format json | s3sniff-emit --to stix
    s3sniff-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('bucket', '') + ' ' + rec.get('acl', '') + ' ' + rec.get('policy', '')
        out['severity'] = 'info'
        out['type'] = 'acl-policy'
        out['description'] = 'ACL or policy found in bucket listing'
        out['tags'] = ['s3', 'acl', 'policy']
