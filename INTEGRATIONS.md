...
**s3sniff** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `s3sniff` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
s3sniff ... --format json | cognis-connect emit --to sigma   # Sigma rules
s3sniff ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
s3sniff ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
s3sniff ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `s3sniff` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="s3sniff") for rec in records]   # records = your JSON output
